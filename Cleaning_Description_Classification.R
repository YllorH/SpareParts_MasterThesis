...
# 
# The cleaning and classyfying process follow the steps from: https://github.com/danieldehaan96/spdf and https://github.com/KhueNguyenTK/Spare-Part-Demand-Forecasting
# List of data sets:
...
# 
# Author: Yllor Hasani, <640757yh@eur.nl>

# LOADING DATA
load("C:\\Users\\yllor\\OneDrive\\Bureau\\Thesis\\SpareParts\\RawData.RData")

########## DATA CLEANING ##########

# Extracting price & leadtime vectors (leadtimes are set to 0)
...
# Correlation coefficient between average demand and average product price
corMAN <- cor.test(avgSalesPerItem_MAN,pricesMAN)
corBRAF <- cor.test(avgSalesPerItem_BRAF, pricesBRAF)
corAUTO <- cor.test(avgSalesPerItem_AUTO, pricesAUTO)
corOIL <- cor.test(avgSalesPerItem_OIL, pricesOIL)
...
# Correlation coefficient between average demand and average product price
corSim1 <- cor.test(avgSalesPerItem_SIM1,pricesSIM1)

corSim2 <- cor.test(avgSalesPerItem_SIM2,pricesSIM2)

corSim3 <- cor.test(avgSalesPerItem_SIM3,pricesSIM3)

corSim4 <- cor.test(avgSalesPerItem_SIM4,pricesSIM4)
...
simulated$CorrelationDemand_Price <- c(corSim1$estimate, corSim2$estimate, corSim3$estimate, corSim4$estimate)
# View the tables
View(industrial)
View(simulated)
